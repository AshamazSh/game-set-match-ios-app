...
V2's safe Nullify relationship for `Match.rule` is retained. Existing shared rules survive another match's deletion. New matches own independent rule snapshots; the repository removes rules only when no remaining match references them. Migrations do not reset or replace user stores. Migration tests create actual V1/V2/V3/V4 SQLite stores, reopen them as V5 and verify continued scoring and replay.
...
The Score/Stats menu opens statistics for the set currently shown in Score, or whole-match statistics when the match is complete. Arrow navigation follows whole match, set 1, set 2, etc. Statistics use recorded point winners and servers; no database migration or separate counters are required. Undo recomputes the values.

Team tables show own serve, right/left serve, break-point conversion, completed games that reached 40:40, and mini-breaks. Each metric has won/total/percentage columns. Deciding Star/Golden rallies are included in own serve but excluded from both side-specific counters. Service side follows rally parity, reset for each game and continuous across server changes in tiebreaks. Break-point opportunities are evaluated before each played rally and exclude tiebreaks. Both normal and super tiebreaks contribute mini-breaks. A completed deuce game counts once. Zero denominators display a dash.

Higher team percentages are highlighted using unrounded ratios. Break points instead highlight won and total independently; ties are unhighlighted. In doubles, the Players tab shows only each player's recorded service statistics because the receiver is not stored. Singles team statistics are also personal statistics. Statistics tests cover service attribution and sides, deciding-point exclusions, break opportunities, deuce games, both tiebreak types, weighted match totals, highlighting, migration, save failures, and undo. UI tests cover period navigation, completed-match entry, returning to Score, and team/player switching. Debug-only statistics fixtures require `--ui-testing` and seed matches through normal scoring into an isolated in-memory store.

## First serving team

New matches (including replay) start with two team choices on the match screen, available on both iPhone and Watch. This is match state, not a rule or creation-form option. V5 adds `Match.firstServingTeam` (zero-based, default 0) and `Match.requiresFirstServerSelection` (default false for migrated matches, explicitly true on creation). All existing matches, including those without points, retain team 1 as their initial server without a prompt. The chosen team's first player starts; choosing team 2 swaps the team order while preserving player order: B1, A1, B2, A2. The same order carries through sets and tiebreaks.

Selection is a single persisted transaction that advances the revision. Scoring is blocked until selection, and simultaneous/repeated choices use the existing command revision and request-ID protections. Undoing every point keeps the selection; replay requests a fresh choice. Optional snapshot fields keep older messages decodable. An older Watch app can continue existing matches; for a newly created pending match, the choice must be made on the updated phone before scoring from the old Watch. Statistics and Game Log use the recorded server as before.
